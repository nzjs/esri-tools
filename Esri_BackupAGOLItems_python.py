# This script downloads data as a JSON file from a specified item in portal or downloads all items in a portal site and zips these up.
# Adapted from a script developed by Eagle Technologies
...
logFile = os.path.join(os.path.dirname(__file__), "BackupArcGISOnline-new.log") # e.g. os.path.join(os.path.dirname(__file__), "Example.log")
# Email logging
...
def mainFunction(portalURL,portalUser,portalPassword,itemID,downloadLocation): # Add parameters sent to the script here e.g. (var1 is 1st parameter,var2 is 2nd parameter,var3 is 3rd parameter)
    try:
        ...
        # Else get all items
            ...
            if not os.path.exists(os.path.join(downloadLocation,"AGSOnlineBackup-" + time.strftime("%Y%m%d"))):
                os.makedirs(os.path.join(downloadLocation,"AGSOnlineBackup-" + time.strftime("%Y%m%d")))
            downloadLocation = os.path.join(downloadLocation,"AGSOnlineBackup-" + time.strftime("%Y%m%d"))
